            ...
            <div class="formBox">
                ...
                <ul class="labBox">
                    ...
                    <li>
                        ...
                        <Input v-model="actualfrom" placeholder="如：想周三来面谈" style="width: 100%;" />
                    </li>
                </ul>
                <ul class="labBox">
                    <li>
                    ...
                    <li style="margin-right:0;">
                        ...
                        <Input ref='tagInpt' @click.native="focuTags" v-if="!showDown" @on-blur="blurTages" v-model="tipRemark" placeholder="以顿号分隔，如：标化神、拖延狗、活动强等..." style="width: 100%;" />
                ...
                <ul class="labBox" style="margin-top:40px;">
                    ...
                    <!-- <li v-if="salewish==1">
                        ...
                        <Input v-model="reason" placeholder="如：tt推荐，过往学生推荐 等..." style="width: 100%;" />
                    </li> -->
                </ul>
                ...
                <div class="btmBox"><Button type="success" @click="submitData">确定</Button></div>
            </div>
        </div>
...
export default {
    data() {
        return {
            ...
            highschoolareacode:0,//居住地或高中所在地id
            highschool:'',//高中名称
            yearApplay: '',//申请年份
            yearList:[
                {
    ...
    methods: {
        ...
        getBaseInfo(){
            this.$https.lookMenteeInfo({
                ...
                if(res){
                    this.contName = res.customer.contactname;
                    this.contype = res.customer.contactidentity;
                    this.menteeName = res.customer.chinesename;
                    this.clientType = res.customer.type;
                    this.wechatNum = res.customer.wechatcontact;
                    this.phoneNum = res.customer.phone;
                    this.menteeWechart = res.customer.wechatstudent;
                    this.highschool = res.customer.highschool;
                    this.areacode = res.customer.areacode;
                    this.highschoolareacode = res.customer.hschoolareacode;
                    this.yearApplay = res.customer.appyear;
                    this.comment = res.customer.comment;
                    this.tipRemark = res.customer.tag;
                    this.demand = res.customer.demand;
                    this.comefrom = res.customer.comefrom;
                    this.actualfrom = res.customer.actualfrom;
                    this.salewish = String(res.customer.salewish);
                    this.reason = res.customer.salewishreason;
                }else{
        ...
        ruleFormate(){
            ...
            //     return false;
            // };
            if(/^\s*$/.test(menteeName)||!clientType||/^\s*$/.test(contName)||!contype||/^\s*$/.test(phoneNum)||!yearApplay){
                return false;
            };
        ...
        submitData(){
            ...
            const params = {
                ...
                reason:this.salewishreason
            };
            this.$https.editcustomer(params).then(res=>{
                if(res&&res.code==200){
