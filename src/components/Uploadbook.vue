        ...
        <div class="btmcontbox">
            ...
            <div class="schlistbox">
                <div class="scItem clearfix" v-for="(v,i) in schlist" :key="i">
                    <span class="schName">
                        <label class="upBox" v-if="v.fileCount&&v.fileCount!=v.filePath.length" :for="'upFie_'+v.roundId">
                            <input type="file" :id="'upFie_'+v.roundId" multiple class="upIpt" @change="upOnlyfile($event,v)">
                        ...
                        <div class="progessbox" v-if="v.fileCount">
                            <span :class="['itmpro',v.fileCount==v.filePath.length?'green':k<v.filePath.length?'blue':'']" v-for="(vm,k) in v.fileCount" :key="k"></span>
                        </div>
                    </span>
                    <span class="protig" v-if="v.fileCount">{{v.filePath.length+'/'+v.fileCount}}</span>
                    <span class="rtopticon">
                        <span v-if="!v.fileCount">无额外材料</span>
                        <span v-if="v.fileCount&&!v.filePath.length">暂未上传</span>
                        <span v-else v-for="(item,j) in v.filePath" :key="j">
        ...
        upOnlyfile(e,data){
            ...
            filesList.forEach(v=>{
                ...
                if(file.type=='application/msword'||file.type=='application/vnd.openxmlformats-officedocument.wordprocessingml.document'||file.type=='application/pdf'||file.type=='application/zip'||laststr.indexOf('doc')!=-1){
                    ...
                    this.$https.uponlyschfile(formdata).then(res=>{
                        this.$Spin.hide();
                        e.target.value = '';
                        if(res&&res.code==200){
                            this.getData();
